...
Production authentication flows use server-issued challenges. The browser first requests a short-lived `server_token` bound to the business action, with a budget of five SDK initialization attempts, then submits the resulting `pt_` `captcha_token` to the protected endpoint. Administrators can optionally bind the server token to the issuing client IP; this is disabled by default for multi-CDN, IPv4/IPv6, and proxy deployments. Sub2API validates that token with CaptchaLa and strictly matches its action. The App Secret is never exposed through public settings or frontend code, and validation fails closed on invalid tokens, action mismatches, or provider/network errors.
