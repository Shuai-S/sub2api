    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          inviterRegistrationRewardHint: 'Added to the inviter’s available affiliate quota when a referred user completes signup. 0 = disabled.',
          inviteeRegistrationReward: 'Invitee Signup Reward',
          inviteeRegistrationRewardHint: 'Recorded as affiliate quota and immediately transferred to the new user’s account balance. 0 = disabled.',
      ...
      upstreamBillingProbe: {
        ...
        description: 'Periodically retrieve billing rates declared by upstream Sub2API sites connected to supported provider API keys.',
      ...
      customMenu: {
        title: 'Custom Navigation Entries',
        description: 'Configure sidebar pages and header modal buttons separately. Built-in system entries keep their existing order.',
        sidebarTitle: 'Sidebar Pages',
        sidebarDescription: 'External pages can be embedded or opened in a new tab. Markdown pages always open inside the app.',
        headerTitle: 'Header Modal Buttons',
        headerDescription: 'Buttons appear before announcements and load protected Markdown content when clicked.',
        itemLabel: 'Menu Item #{n}',
        name: 'Menu Name',
        namePlaceholder: 'e.g. Help Center',
        headerNamePlaceholder: 'e.g. Usage Notice',
        url: 'Page URL',
        urlPlaceholder: 'https://example.com/page or md:guide',
        openMode: 'Open Mode',
        openModeEmbedded: 'Embedded',
        openModeNewTab: 'New Tab',
        openModeHint: 'Markdown pages cannot use new-tab mode.',
        modalTitle: 'Modal Title',
        modalTitlePlaceholder: 'Uses the menu name when left empty',
        modalTitleHint: 'Optional, up to 100 characters.',
        modalContent: 'Modal Content',
        modalContentPlaceholder: 'Enter Markdown content...',
        modalContentHint: 'Required. Supports Markdown and safe HTML, up to 50,000 characters. Use external URLs for images.',
        iconSvg: 'SVG Icon',
        ...
        add: 'Add Menu Item',
        addSidebar: 'Add Sidebar Page',
        addHeader: 'Add Header Modal Button',
      ...
      anthropicAdaptiveScheduler: {
        ...
        parameters: {
          ...
          initialReliability: 'Initial reliability',
          consecutiveFailurePenalty: 'Consecutive failure penalty',
          neutralLatencyScore: 'Neutral latency score',
          capacityProbeLoadThreshold: 'Capacity probe load threshold',
          capacitySuccessThreshold: 'Capacity growth success threshold',
          capacityIncreaseStep: 'Capacity increase step',
          minCapacity: 'Minimum learned capacity',
          capacityFailureThreshold: 'Shrink failure threshold',
          minRecentSamplesForShrink: 'Minimum shrink samples',
          shrinkErrorThreshold: 'Shrink error threshold',
          shrinkFactorSoft: 'Soft shrink factor',
          shrinkFactorHard: 'Hard shrink factor',
          hardShrinkFailureMultiplier: 'Hard shrink failure multiplier',
          learningWindowSeconds: 'Learning window (seconds)',
          cooldownSeconds: 'Shrink cooldown (seconds)',
          successEmaAlpha: 'Success EMA alpha',
          latencyEmaAlpha: 'Latency EMA alpha',
        ...
        tooltips: {
          ...
          consecutiveFailurePenalty: 'Extra reliability penalty applied for every consecutive health failure. Higher values demote repeatedly failing accounts faster.',
          neutralLatencyScore: 'Score used when an account has no latency samples for the current model family, or when all measured latencies are equal, from 0 to 1.',
          capacityProbeLoadThreshold: 'Learned-capacity utilization that permits growth when health conditions pass. A waiting request also triggers a probe.',
          capacitySuccessThreshold: 'Capacity can grow only when the success EMA reaches this value and enough consecutive successes have accumulated.',
          capacityIncreaseStep: 'Concurrency slots added after a successful capacity probe, capped by the account concurrency setting.',
          minCapacity: 'Lowest capacity that shrink learning may reach, capped by the account concurrency setting when that is smaller.',
          capacityFailureThreshold: 'Number of consecutive explicit upstream concurrency-limit failures required before sample error rate may trigger shrink.',
          minRecentSamplesForShrink: 'Minimum capacity samples required in the current learning window before error-rate shrink is allowed.',
          shrinkErrorThreshold: 'Fraction of capacity samples that must be explicit concurrency-limit failures before shrink is allowed.',
          shrinkFactorSoft: 'Capacity retained during normal shrink. For example, 0.85 keeps 85% of the current learned capacity.',
          shrinkFactorHard: 'Capacity retained after sustained failures. On save it is constrained to be no greater than the soft shrink factor.',
          hardShrinkFailureMultiplier: 'Hard shrink applies when consecutive capacity failures reach the shrink failure threshold multiplied by this value.',
          learningWindowSeconds: 'Window used for recent capacity sample and error-rate counters. Recent counters restart when it expires.',
          cooldownSeconds: 'How long capacity growth pauses after shrink. The account remains selectable during cooldown; use 0 to disable cooldown.',
          successEmaAlpha: 'Weight of the latest health sample in the success EMA. Higher values react faster to recent success or failure.',
          latencyEmaAlpha: 'Weight of the latest successful request in first-token and total-latency EMAs. Higher values react faster to recent latency changes.',
      ...
      geminiAdaptiveScheduler: {
        title: 'Gemini adaptive scheduler',
        description: 'Disabled by default. Schedules Gemini and Antigravity accounts together while learning reliability, quota pace, dynamic capacity, latency, and cost.',
        mode: 'Run mode',
        modes: {
          shadow: 'Shadow observe',
          enforce: 'Enforce routing'
        },
        stickyEscape: 'Migrate saturated sticky sessions',
        diagnosticLog: 'Diagnostic logs',
        sections: {
          selection: 'Selection and priors',
          capacity: 'Capacity learning',
          learningAndWeights: 'EMA and six-dimension weights',
          diagnostics: 'Diagnostic sampling'
        },
        parameters: {
          topK: 'Top K candidates',
          ...
          consecutiveFailurePenalty: 'Consecutive failure penalty',
          neutralLatencyScore: 'Neutral latency score',
          neutralQuotaScore: 'Neutral quota score',
          minCostMultiplier: 'Minimum cost multiplier',
          capacityProbeLoadThreshold: 'Capacity probe load threshold',
          capacitySuccessThreshold: 'Capacity growth success threshold',
          capacityIncreaseStep: 'Capacity increase step',
          minCapacity: 'Minimum learned capacity',
        ...
        tooltips: {
          mode: 'Shadow mode calculates and records adaptive decisions without changing existing routing. Enforce mode keeps Priority tiers strict, then selects with Top K and Softmax inside a tier.',
          stickyEscape: 'When off, an available session binding is always retained. When on, a binding at learned capacity can migrate under a lease; only the first successful response commits the new binding.',
          diagnosticLog: 'Emits scheduling-decision and score diagnostics. Successful requests use the configured sample rate while failures and important state changes may still be recorded.',
          topK: 'Number of highest-scoring candidates entering Softmax selection in each Priority tier. Remaining accounts stay available as ordered fallbacks.',
          softmaxTemperature: 'Controls randomness inside Top K. Lower values favor the best score; higher values spread traffic more evenly.',
          initialReliability: 'Reliability prior used before an account has path-success samples, from 0 to 1.',
          consecutiveFailurePenalty: 'Extra reliability penalty for consecutive health failures. Higher values demote failing accounts faster.',
          neutralLatencyScore: 'Latency score used when the current model family has no useful latency comparison, from 0 to 1.',
          neutralQuotaScore: 'Quota-pace score used when a local Gemini quota snapshot is unavailable, from 0 to 1.',
          minCostMultiplier: 'Lower bound applied to rate_multiplier for cost scoring so anomalously low values cannot gain unbounded advantage.',
          capacityProbeLoadThreshold: 'Learned-capacity utilization that permits recovery probes when health conditions pass.',
          capacitySuccessThreshold: 'Success EMA required before a successful capacity probe can increase learned capacity.',
          capacityIncreaseStep: 'Concurrency slots restored after a healthy probe, capped by configured account concurrency.',
          minCapacity: 'Lowest learned capacity allowed after explicit upstream concurrency-limit shrink events.',
          capacityFailureThreshold: 'Consecutive explicit upstream concurrency-limit failures required before shrink. Generic 429s, model-capacity errors, and local queuing do not count.',
          minRecentSamplesForShrink: 'Minimum capacity samples required in the learning window before error-rate shrink is allowed.',
          shrinkErrorThreshold: 'Fraction of recent capacity samples that must be explicit concurrency failures before shrink.',
          shrinkFactorSoft: 'Capacity retained during normal shrink.',
          shrinkFactorHard: 'Capacity retained after sustained concurrency failures; normalization keeps it no greater than the soft factor.',
          hardShrinkFailureMultiplier: 'Hard shrink applies when consecutive capacity failures reach the failure threshold multiplied by this value.',
          learningWindowSeconds: 'Rolling window for recent health and capacity samples.',
          cooldownSeconds: 'How long capacity growth pauses after shrink; use 0 to disable cooldown.',
          successEmaAlpha: 'Weight of the latest success or failure in path and model-family success EMAs.',
          latencyEmaAlpha: 'Weight of the latest successful request in first-token and total-latency EMAs.',
          weightReliability: 'Weight for reliability derived from path success, model-family success, and failure streaks.',
          weightQuota: 'Weight for remaining pace across Pro, Flash, shared, and unlimited quota buckets.',
          weightCapacity: 'Weight for remaining learned concurrency capacity.',
          weightLatency: 'Weight for relative first-token and total latency in the current model family.',
          weightCost: 'Weight for cost derived from the account rate_multiplier.',
          weightExploration: 'Weight rewarding low-sample accounts. Defaults are restored if all six weights are zero.',
      ...
      openaiAdaptiveScheduler: {
        ...
        failureThreshold: 'Capacity failure threshold',
        cooldownBase: 'Base cooldown seconds',
        cooldownMax: 'Max cooldown seconds',
        burstAndShrink: 'Burst and shrink protection',
        minRecentSamplesForShrink: 'Min shrink samples',
        shrinkErrorThreshold: 'Shrink error threshold',
        shrinkFactorSoft: 'Soft shrink factor',
        shrinkFactorHard: 'Hard shrink factor',
        ...
        errorEma: 'Error EMA',
        latencyEma: 'Latency EMA',
        ttftEma: 'TTFT EMA',
        weights: 'Scoring weights',
        weightSuccess: 'Success',
        weightCost: 'Cost',
        weightCapacity: 'Remaining capacity',
        ...
        tooltips: {
          ...
          minCostMultiplier: 'Lower bound for account cost multiplier, preventing empty or very low rate_multiplier values from making cost score unrealistically large.',
          thompson: 'Enables Thompson Sampling to explore when success-rate confidence is low. Useful when provider concurrency changes dynamically.',
          ...
          failureThreshold: 'Consecutive failures required before capacity reduction or cooldown is triggered.',
          cooldownBase: 'Base cooldown seconds after account failures. Short cooldowns allow quicker recovery probing.',
          cooldownMax: 'Maximum cooldown seconds after repeated failures, limiting cooldown growth.',
          burstAndShrink: 'Protection parameters for sudden traffic spikes, dynamic provider concurrency, and occasional upstream errors.',
          minRecentSamplesForShrink: 'Minimum number of samples in the learning window before down-scaling is allowed. Higher values reduce false shrink from occasional errors.',
          ...
          errorEma: 'Update weight for feeding new error samples into historical error rate.',
          latencyEma: 'Update weight for feeding total request latency samples into historical latency.',
          ttftEma: 'Update weight for feeding streaming first-token latency samples into historical TTFT.',
          weights: 'Controls the importance of each factor in the final score. Higher weight means stronger impact on account selection.',
          weightSuccess: 'Success-rate weight. Higher values prefer accounts with better recent success rate.',
          weightCost: 'Cost weight. Higher values prefer accounts with lower rate_multiplier.',
          weightCapacity: 'Remaining-capacity weight. Higher values prefer accounts with more learned spare capacity.',
          weightLatency: 'Latency weight. Higher values prefer accounts with faster responses and lower TTFT.',
          weightStability: 'Stability weight. Higher values prefer accounts with lower historical variance and fewer errors.',
          weightExploration: 'Exploration weight. Higher values give more traffic to under-sampled or uncertain accounts for continued learning.'
        }
      },
      usageRecords: {
        title: 'Usage Records',
