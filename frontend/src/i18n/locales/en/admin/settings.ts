    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          inviterRegistrationReward: 'Inviter Registration Reward',
          inviterRegistrationRewardHint: 'Quota granted to the inviter when an invited user completes registration. 0 = disabled.',
          inviteeRegistrationReward: 'Invitee Registration Reward',
          inviteeRegistrationRewardHint: 'Quota granted to the invited user when registration is completed. 0 = disabled.',
      ...
      customMenu: {
        ...
        description: 'Add custom pages to the sidebar or modal entries to the header. Each item can be visible to regular users or administrators.',
        sidebarTitle: 'Sidebar Pages',
        sidebarDescription: 'Open a URL or Markdown page from the sidebar navigation.',
        headerTitle: 'Header Modals',
        headerDescription: 'Open custom Markdown content in a modal from the page header.',
        itemLabel: 'Menu Item #{n}',
        name: 'Menu Name',
        namePlaceholder: 'e.g. Help Center',
        headerNamePlaceholder: 'e.g. Notice',
        url: 'Page URL',
        urlPlaceholder: 'https://example.com/page',
        openMode: 'Open Mode',
        openModeEmbedded: 'Embedded Page',
        openModeNewTab: 'New Tab',
        openModeHint: 'Markdown pages always open inside the application.',
        modalTitle: 'Modal Title',
        modalTitlePlaceholder: 'e.g. Service Notice',
        modalTitleHint: 'Leave empty to use the menu name.',
        modalContent: 'Modal Content',
        modalContentPlaceholder: 'Enter Markdown content...',
        modalContentHint: 'Markdown formatting is supported.',
        iconSvg: 'SVG Icon',
      ...
      anthropicAdaptiveScheduler: {
        ...
        description: 'Disabled by default. Keeps sticky routing first, then scores accounts by reliability, available capacity, TTFT, and dynamic cost.',
        mode: 'Run mode',
        modes: {
        ...
        parameters: {
          ...
          learningMinHealthSamples: 'Health samples to learn',
          cooldownSeconds: 'Shrink cooldown (seconds)',
          cooldownMaxSeconds: 'Maximum circuit cooldown (seconds)',
          successEmaAlpha: 'Success EMA alpha',
          ttftEmaAlpha: 'TTFT EMA alpha',
          healthFailureThreshold: 'Account failure threshold',
          highErrorMinSamples: 'High-error minimum samples',
          highErrorMaxSamples: 'High-error sample cap',
          highErrorEnterRate: 'High-error entry rate',
          highErrorExitRate: 'High-error recovery rate',
          capacityShrinkFactor: 'Capacity shrink factor',
          capacityGrowthFactor: 'Capacity recovery factor',
          capacityRecoverySamples: 'Capacity recovery successes',
          capacityRecoveryLoad: 'Capacity recovery load',
          quotaProbeIntervalSeconds: 'Quota probe interval (seconds)',
          sameAccount429RetryBudgetMs: 'Same-account 429 wait budget (ms)',
        ...
        tooltips: {
          mode: 'Shadow mode learns and computes decisions without changing routing. Enforce mode uses the adaptive order after sticky routing is unavailable.',
          ...
          topK: 'This many highest-scoring accounts enter weighted Softmax selection. Remaining accounts stay available as score-ordered fallbacks.',
          ...
          explorationRate: 'Probability that a fresh selection explores an unlearned or learning non-OAuth account. OAuth accounts do not participate in exploration, and exploration does not impose a one-request cap.',
          consecutiveFailurePenalty: 'Extra reliability penalty applied for every consecutive account failure. Higher values demote repeatedly failing accounts faster.',
          learningWindowSeconds: 'Sliding window used to determine account learning and high-error state.',
          learningMinHealthSamples: 'Health observations required inside the sliding window before an account is learned.',
          cooldownSeconds: 'Initial account-circuit cooldown and the capacity recovery delay after shrink.',
          cooldownMaxSeconds: 'Maximum exponential account-circuit cooldown after failed half-open probes.',
          ...
          ttftEmaAlpha: 'Weight of the latest successful first-token latency sample.',
          healthFailureThreshold: 'Unique consecutive account failures required to open the account circuit. Authentication failures open it immediately.',
          highErrorMinSamples: 'Minimum sliding-window sample count before high-error status can be entered.',
          highErrorMaxSamples: 'Maximum recent health observations retained for high-error calculation.',
          highErrorEnterRate: 'Failure rate that enters high-error status.',
          highErrorExitRate: 'Failure rate below which high-error status is cleared.',
          capacityShrinkFactor: 'Capacity retained after an explicit upstream capacity-limit event. Existing in-flight requests are never cancelled.',
          capacityGrowthFactor: 'Capacity multiplier applied after enough successfully admitted requests in the current generation.',
          capacityRecoverySamples: 'Successful requests in the current capacity generation required before recovery.',
          capacityRecoveryLoad: 'Legacy compatibility setting; capacity recovery no longer depends on utilization.',
          quotaProbeIntervalSeconds: 'Retry interval for quota-limited accounts when no reset time is supplied.',
      ...
      geminiAdaptiveScheduler: {
        ...
        description: 'Disabled by default. Saturated sticky sessions escape automatically, then accounts are scored by reliability, available capacity, TTFT, and dynamic cost.',
        mode: 'Run mode',
        modes: {
          shadow: 'Shadow observe',
          enforce: 'Enforce routing'
        ...
        parameters: {
          ...
          softmaxTemperature: 'Softmax temperature',
          explorationRate: 'Exploration rate',
          recoveryExplorationRate: 'Recovery exploration rate',
          recoveryMaxConcurrency: 'Recovery global concurrency',
          recoveryWarmupSuccesses: 'Recovery warmup successes',
          consecutiveFailurePenalty: 'Consecutive failure penalty',
          learningWindowSeconds: 'Learning window (seconds)',
          learningMinHealthSamples: 'Health samples to learn',
          cooldownSeconds: 'Shrink cooldown (seconds)',
        ...
        tooltips: {
          ...
          topK: 'Number of highest-scoring candidates entering Softmax selection. Remaining accounts stay available as ordered fallbacks.',
          ...
          explorationRate: 'Probability that a fresh selection explores an unlearned or learning non-OAuth account. OAuth accounts do not participate in exploration, and exploration does not impose a one-request cap.',
          consecutiveFailurePenalty: 'Extra reliability penalty for consecutive account failures. Higher values demote failing accounts faster.',
          learningWindowSeconds: 'Sliding window used to determine account learning and high-error state.',
          learningMinHealthSamples: 'Health observations required inside the sliding window before an account is learned.',
          cooldownSeconds: 'Initial account-circuit cooldown and the capacity recovery delay after shrink.',
          cooldownMaxSeconds: 'Maximum exponential account-circuit cooldown after failed half-open probes.',
          successEmaAlpha: 'Weight of the latest account health observation in the success EMA.',
          ttftEmaAlpha: 'Weight of the latest successful first-token latency sample.',
          healthFailureThreshold: 'Unique consecutive account failures required to open the account circuit. Authentication failures open it immediately.',
          highErrorMinSamples: 'Minimum sliding-window sample count before high-error status can be entered.',
          highErrorMaxSamples: 'Maximum recent health observations retained for high-error calculation.',
      ...
      openaiAdaptiveScheduler: {
        ...
        description: 'Disabled by default. Keeps previous-response and session stickiness first, then scores accounts by reliability, available capacity, TTFT, and dynamic cost.',
        mode: 'Mode',
        modes: {
        ...
        parameters: {
          ...
          ttftEmaAlpha: 'TTFT EMA alpha',
          healthFailureThreshold: 'Account failure threshold',
          cooldownSeconds: 'Initial circuit cooldown (seconds)',
          cooldownMaxSeconds: 'Maximum circuit cooldown (seconds)',
          highErrorMinSamples: 'High-error minimum samples',
          highErrorMaxSamples: 'High-error sample cap',
          highErrorEnterRate: 'High-error entry rate',
        ...
        tooltips: {
          ...
          diagnosticLog: 'Emits structured scheduler diagnostics. Successful decisions use the sample rate; failures and important state changes may be forced.',
          topK: 'This many highest-scoring accounts enter weighted Softmax selection. Remaining accounts stay available as score-ordered fallbacks.',
          softmaxTemperature: 'Controls randomness inside Top K. Lower values favor the best score; higher values spread traffic more evenly.',
          explorationRate: 'Probability that a fresh selection explores an unlearned or learning API-key account. OAuth accounts do not participate in exploration, and exploration does not impose a one-request cap.',
          recoveryExplorationRate: 'Probability that a non-sticky fresh request enters the independent recovery channel. Set to 0 to disable it. OAuth hard priority is preserved.',
          recoveryMaxConcurrency: 'Maximum concurrent recovery probes per instance. Each account is always limited to one recovery probe.',
          recoveryWarmupSuccesses: 'Successful recovery samples required before a warming account returns to normal active status.',
          consecutiveFailurePenalty: 'Extra reliability penalty for each consecutive account failure.',
          learningWindowSeconds: 'Sliding window used to determine account learning and high-error state.',
          learningMinHealthSamples: 'Health observations required inside the sliding window before an account is learned.',
          successEmaAlpha: 'Weight of the latest account health observation in the success EMA.',
          ttftEmaAlpha: 'Weight of the latest successful first-token latency sample.',
          ...
          capacityRecoveryLoad: 'Legacy compatibility setting; capacity recovery no longer depends on utilization.',
          quotaProbeIntervalSeconds: 'Retry interval for quota-limited accounts when no reset time is supplied.',
          weightReliability: 'Weight for reliability after the consecutive-failure penalty.',
          weightCapacity: 'Weight for currently available effective capacity.',
          weightTTFT: 'Weight for TTFT. It is redistributed when there are not enough comparable samples.',
          weightCost: 'Weight for dynamic candidate-layer cost: lowest available rate divided by account rate.',
          weightCache: 'Cache-hit bonus weight. Only accounts above the mature candidate-layer median are rewarded; lower hit rates are not penalized.',
