    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          inviterRegistrationRewardHint: '被邀请用户完成注册后，发放给邀请人的额度。0 = 不发放。',
          inviteeRegistrationReward: '受邀用户注册奖励',
          inviteeRegistrationRewardHint: '受邀用户完成注册后，发放给该用户的额度。0 = 不发放。',
          rebateRate: '全局返利比例',
      ...
      customMenu: {
        title: '自定义菜单页面',
        description: '在侧边栏添加自定义页面，或在页头添加弹窗入口。每个入口均可设置对普通用户或管理员可见。',
        sidebarTitle: '侧边栏页面',
        sidebarDescription: '从侧边栏导航打开 URL 或 Markdown 页面。',
        headerTitle: '页头弹窗',
        headerDescription: '从页头入口以弹窗形式展示自定义 Markdown 内容。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
        namePlaceholder: '如：帮助中心',
        headerNamePlaceholder: '如：公告',
        url: '页面 URL',
        urlPlaceholder: 'https://example.com/page',
        openMode: '打开方式',
        openModeEmbedded: '内嵌页面',
        openModeNewTab: '新标签页',
        openModeHint: 'Markdown 页面始终在应用内打开。',
        modalTitle: '弹窗标题',
        modalTitlePlaceholder: '如：服务公告',
        modalTitleHint: '留空时使用菜单名称。',
        modalContent: '弹窗内容',
        modalContentPlaceholder: '输入 Markdown 内容...',
        modalContentHint: '支持 Markdown 格式。',
        iconSvg: 'SVG 图标',
      ...
      anthropicAdaptiveScheduler: {
        ...
        description: '默认关闭。优先保留粘性路由；需要重新选择时，按账号可靠性、可用容量、首字延迟和动态成本评分。',
        mode: '运行模式',
        modes: {
          shadow: 'Shadow 观察',
          enforce: 'Enforce 执行'
        ...
        parameters: {
          ...
          learningMinHealthSamples: '学成所需健康样本',
          successEmaAlpha: '成功率 EMA Alpha',
          ttftEmaAlpha: '首字延迟 EMA Alpha',
          healthFailureThreshold: '账号失败熔断阈值',
          cooldownSeconds: '初始熔断冷却（秒）',
          cooldownMaxSeconds: '最大熔断冷却（秒）',
          highErrorMinSamples: '高错误最少样本数',
          highErrorMaxSamples: '高错误样本上限',
          highErrorEnterRate: '高错误进入阈值',
          highErrorExitRate: '高错误恢复阈值',
          capacityShrinkFactor: '容量收缩系数',
          capacityGrowthFactor: '容量恢复系数',
          capacityRecoverySamples: '容量恢复成功数',
          capacityRecoveryLoad: '容量恢复负载阈值',
          quotaProbeIntervalSeconds: '配额探测间隔（秒）',
          sameAccount429RetryBudgetMs: '同账号 429 等待预算（毫秒）',
        ...
        tooltips: {
          mode: 'Shadow 只观察和学习，不改变真实路由；Enforce 在粘性路由不可用时使用学习结果选择账号。',
          diagnosticLog: '输出结构化调度诊断。成功决策按采样率记录，失败、旁路和关键状态变化可强制记录，不包含请求正文或账号凭据。',
          topK: '综合评分最高的候选中，有多少个进入 Softmax 加权选择；其余候选仍按得分作为后备。',
          softmaxTemperature: '控制 Top K 内的随机程度。值越小越偏向最高分账号，值越大流量越分散。',
          explorationRate: '新选择探索未学成非 OAuth 账号的概率。OAuth 账号不参与探索，也不受单探索请求限制。',
          consecutiveFailurePenalty: '每次连续账号失败对可靠性分的额外惩罚。值越大，失败账号降权越快。',
          learningWindowSeconds: '判断账号学习状态和高错误状态使用的滑动窗口。',
          learningMinHealthSamples: '滑动窗口内达到此健康观察数后，账号进入已学习状态。',
          successEmaAlpha: '最新账号健康观察在成功率 EMA 中的权重。',
          ttftEmaAlpha: '最新成功请求首字延迟样本在 EMA 中的权重。',
          healthFailureThreshold: '连续多少个去重账号失败后打开账号熔断；认证失败首次即打开。',
          cooldownSeconds: '账号熔断的初始冷却时长，也是缩容后的容量恢复等待时间。',
          cooldownMaxSeconds: '半开探测失败后指数延长熔断时长的上限。',
          highErrorMinSamples: '进入高错误状态前，滑动窗口至少需要的样本数。',
          highErrorMaxSamples: '计算高错误率时保留的近期健康观察上限。',
          highErrorEnterRate: '达到此失败率时进入高错误状态。',
          highErrorExitRate: '失败率降到此值或以下时退出高错误状态。',
          capacityShrinkFactor: '明确上游容量限制发生后保留的容量比例；不会取消已有在途请求。',
          capacityGrowthFactor: '当前容量代次积累足够实际准入成功后使用的恢复倍数。',
          capacityRecoverySamples: '当前容量代次恢复前所需的实际准入成功请求数。',
          capacityRecoveryLoad: '旧版本兼容设置；容量恢复不再依赖利用率。',
          quotaProbeIntervalSeconds: '上游未给出重置时间时，配额受限账号的重试探测间隔。',
          weightReliability: '连续失败惩罚后账号可靠性在总分中的权重。',
      ...
      geminiAdaptiveScheduler: {
        title: 'Gemini 自适应调度',
        description: '默认关闭。粘性账号满载时自动逃逸，并按账号可靠性、可用容量、首字延迟和动态成本评分。',
        mode: '运行模式',
        modes: {
          shadow: 'Shadow 观察',
          enforce: 'Enforce 执行'
        ...
        parameters: {
          ...
          softmaxTemperature: 'Softmax 温度',
          explorationRate: '探索比例',
          recoveryExplorationRate: '恢复探索比例',
          recoveryMaxConcurrency: '恢复探索全局并发',
          recoveryWarmupSuccesses: '恢复预热成功数',
          consecutiveFailurePenalty: '连续失败惩罚',
          learningWindowSeconds: '学习窗口（秒）',
          learningMinHealthSamples: '学成所需健康样本',
          successEmaAlpha: '成功率 EMA Alpha',
        ...
        tooltips: {
          mode: 'Shadow 只观察和学习，不改变真实路由；Enforce 在粘性路由不可用时使用学习结果选择账号。',
          diagnosticLog: '输出结构化调度诊断。成功决策按采样率记录，失败和关键状态变化仍可完整记录。',
          topK: '综合评分最高的候选中，有多少个进入 Softmax 加权选择；其余候选仍按得分作为后备。',
          softmaxTemperature: '控制 Top K 内随机程度。值越小越偏向最高分，值越大流量越分散。',
          explorationRate: '新选择探索未学成非 OAuth 账号的概率。OAuth 账号不参与探索，也不受单探索请求限制。',
          consecutiveFailurePenalty: '每次连续账号失败对可靠性分的额外惩罚，值越大降权越快。',
          learningWindowSeconds: '判断账号学习状态和高错误状态使用的滑动窗口。',
          learningMinHealthSamples: '滑动窗口内达到此健康观察数后，账号进入已学习状态。',
          successEmaAlpha: '最新账号健康观察在成功率 EMA 中的权重。',
          ttftEmaAlpha: '最新成功请求首字延迟样本在 EMA 中的权重。',
      ...
      openaiAdaptiveScheduler: {
        title: 'OpenAI 自适应调度',
        description: '默认关闭。优先保留 previous response 和会话粘性；需要重新选择时，按账号可靠性、可用容量、首字延迟和动态成本评分。',
        mode: '运行模式',
        modes: {
          shadow: 'Shadow 观察',
          enforce: 'Enforce 执行'
        ...
        tooltips: {
          mode: 'Shadow 只观察和学习，不改变真实路由；Enforce 使用学习结果选择 OpenAI 账号。',
          diagnosticLog: '输出结构化调度诊断。成功决策按采样率记录，失败和关键状态变化可强制记录。',
          topK: '综合评分最高的候选中，有多少个进入 Softmax 加权选择；其余候选仍按得分作为后备。',
          softmaxTemperature: '控制 Top K 内的随机程度。值越小越偏向最高分，值越大流量越分散。',
          explorationRate: '新选择探索未学成 API Key 账号的概率。OAuth 账号不参与探索，也不受单探索请求限制。',
          recoveryExplorationRate: '无既有粘性的新请求进入独立恢复探索通道的概率；设为 0 可关闭。恢复探索仍保持 OAuth 硬优先。',
          recoveryMaxConcurrency: '同一实例允许同时进行的恢复探测数；每个账号始终最多 1 个恢复探测。',
          recoveryWarmupSuccesses: '恢复账号连续获得这些成功样本后，从预热状态回到正常候选状态。',
          consecutiveFailurePenalty: '每次连续账号失败对可靠性分的额外惩罚。',
          learningWindowSeconds: '判断账号学习状态和高错误状态使用的滑动窗口。',
          learningMinHealthSamples: '滑动窗口内达到此健康观察数后，账号进入已学习状态。',
          successEmaAlpha: '最新账号健康观察在成功率 EMA 中的权重。',
          ttftEmaAlpha: '最新成功请求首字延迟样本在 EMA 中的权重。',
