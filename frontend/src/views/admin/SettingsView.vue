            ...
            <div class="space-y-5 p-6">
              ...
              >
                ...
                <div class="mb-6">
                  ...
                  <div
                    class="grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1 lg:grid-cols-4 dark:bg-dark-700"
                  >
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      data-testid="captcha-provider-captchala"
                      class="inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition"
                      :class="
                        captchaProviderSelection === 'captchala'
                          ? 'bg-white text-primary-700 shadow-sm dark:bg-dark-800 dark:text-primary-300'
                          : 'text-gray-600 hover:text-gray-900 dark:text-dark-300 dark:hover:text-white'
                      "
                      @click="selectCaptchaProvider('captchala')"
                    >
                      {{ t("admin.settings.captcha.providerCaptchaLa") }}
                    </button>
                  </div>
                </div>
                ...
                <div v-else class="grid grid-cols-1 gap-6">
                  <div>
                    <label
                      class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                    >
                      {{ t("admin.settings.captchala.appKey") }}
                    </label>
                    <input
                      v-model="form.captchala_app_key"
                      type="text"
                      autocomplete="off"
                      class="input font-mono text-sm"
                      placeholder="pk_..."
                    />
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                      {{ t("admin.settings.captchala.appKeyHint") }}
                    </p>
                  </div>
                  <div>
                    <label
                      class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                    >
                      {{ t("admin.settings.captchala.appSecret") }}
                    </label>
                    <input
                      v-model="form.captchala_app_secret"
                      type="password"
                      autocomplete="new-password"
                      class="input font-mono text-sm"
                      :placeholder="t('admin.settings.captchala.keepExisting')"
                    />
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                      {{
                        form.captchala_app_secret_configured
                          ? t("admin.settings.captchala.configured")
                          : t("admin.settings.captchala.required")
                      }}
                    </p>
                  </div>
                  <p class="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <div class="flex flex-wrap gap-x-4 gap-y-2 text-sm">
                    <a
                      ...
                      class="text-primary-600 hover:text-primary-500"
                    >
                      {{ t("admin.settings.captchala.openDashboard") }}
                    </a>
                    <a
                      href="https://docs.captcha.la/zh-CN/"
                      target="_blank"
                      rel="noopener noreferrer"
                      class="text-primary-600 hover:text-primary-500"
                    >
                      {{ t("admin.settings.captchala.openDocs") }}
                    </a>
                  </div>
                </div>
              </div>
                ...
                >
                  ...
                  <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <label class="flex items-center gap-2 sm:col-span-2">
                      ...
                      <span class="text-sm text-gray-700 dark:text-gray-300">{{ t("admin.settings.customMenu.hideOpenButton") }}</span>
                    </label>
                  </div>
                </div>
...
const SchedulerParamHelp = defineComponent({
  name: "SchedulerParamHelp",
  props: {
    content: {
      type: String,
      required: true,
  ...
  setup(props) {
    return () => h(HelpTooltip, { content: props.content });
  },
});
...
function applyOpenAIAdaptiveSchedulerRecommendedValues(): void {
  const formRecord = form as unknown as Record<
    OpenAIAdaptiveSchedulerRecommendedKey,
    unknown
  >;
...
type CaptchaProviderSelection = "turnstile" | "tencent" | "aliyun" | "captchala";
...
function applyCaptchaSelection(provider: CaptchaProviderSelection | null): void {
  ...
  form.captchala_enabled = provider === "captchala";
}
...
function syncCaptchaProviderSelection(): void {
  ...
  } else if (form.captchala_enabled) {
    captchaProviderSelection.value = "captchala";
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      custom_menu_items: form.custom_menu_items.map((item) => {
        ...
        // Sidebar is the historical default. Omit its internal defaults from
        // the payload while retaining explicit new-tab and header metadata.
        if (normalized.placement === "sidebar") {
          delete normalized.placement;
          if (normalized.open_mode === "embedded") delete normalized.open_mode;
        }
        return normalized;
      }),
    ...
    for (const [key, value] of Object.entries(updated)) {
      ...
      if (value !== null && value !== undefined) {
        (form as Record<string, unknown>)[key] =
          key === "custom_menu_items" && Array.isArray(value)
            ? value.map((item) => ({ ...item }))
            : value;
      }
    }
