...
func RegisterAuthRoutes(
	...
	auth.Use(gin.HandlerFunc(auditLog))
	{
		...
		auth.POST("/captcha/challenge", rateLimiter.LimitWithOptions("auth-captcha-challenge", 30, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.IssueCaptchaChallenge)
