...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	if resp.StatusCode >= 400 {
		...
		} else if account.Platform != PlatformGrok && shouldCooldownOpenAITransientUpstreamError(resp.StatusCode, respBody) {
			canonicalModel := canonicalOpenAIAccountSchedulingModel(account, originalModel)
			s.handleOpenAIAccountUpstreamError(ctx, account, resp.StatusCode, resp.Header, respBody, canonicalModel)
		}
		...
		if upstreamEventErr != nil {
			if holdRateLimitError {
				return resultWithUsage(), NewOpenAIWSClientCloseError(
					coderws.StatusTryAgainLater,
					"upstream rate limit exceeded, please reconnect",
					fmt.Errorf("upstream http bridge error event: %s", upstreamEventErr.Error()),
				)
			}
