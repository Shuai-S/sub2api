...
type AccountSelectionResult struct {
	...
	// profitGate carries the gate that was active during account selection so
	// handlers can repeat the terminal check after acquiring a slot.
...
type UpstreamFailoverError struct {
	...
	UpstreamErrorCode        string      // 稳定的上游错误码；用于响应体被规范化后的诊断日志
	UpstreamErrorType        string      // 稳定的上游错误类型；用于响应体被规范化后的诊断日志
	ForceCacheBilling        bool        // Antigravity 粘性会话切换时设为 true
	RetryableOnSameAccount   bool        // 临时性错误（如 Google 间歇性 400、空响应），应在同一账号上重试 N 次再切换
	RequestScopedTransient   bool        // 故障因素与账号无关（如上游按客户端身份/模型容量降载）：可同账号重试，但不得据此对账号做临时封禁
	SafeToFailoverAfterWrite bool        // 仅写出 SSE 注释等非语义字节时，仍可在同一客户端流中切换账号
	FirstOutputGuardFailure  bool        // 首输出超时/缓冲保护失败，使用独立的换号次数上限
