...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	for {
		...
		if eventType == "response.failed" {
			...
			if errors.As(failedErr, &failoverErr) && shouldReturnOpenAIWSResponseFailedFailover(message) {
				return resultWithUsage(), failoverErr
			}
			// Policy failures still need to reach the handler so it can persist the
			// cyber-policy usage record. Other terminal failures are represented by
			// UpstreamTerminalEvent and must remain a successful transport result.
