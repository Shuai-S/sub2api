		...
		if reqStream {
			...
			if err != nil {
				if signal, ok := asOpenAICompactFallbackSignal(err); ok {
					...
					// A second compact model failure ends the fallback; account-state
					// failures still use normal failover and proxy attribution.
					modelFallbackExhausted := compactModelFallbackRetried && isOpenAICompactModelFailure(compactResp.StatusCode, signal.message, compactBody)
					if !modelFallbackExhausted && s.shouldFailoverOpenAIUpstreamResponse(account, compactResp.StatusCode, signal.message, compactBody) {
						appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
							ProxyID:            opsUpstreamProxyID(account),
							ProxyName:          opsUpstreamProxyName(account),
							Platform:           account.Platform,
