...
func TestFailoverOpenAIUpstreamHTTPError_NilContextUsesGlobalTransientPolicy(t *testing.T) {
	repo := &tempUnschedulableOpenAIAccountRepo{}
	svc := &OpenAIGatewayService{
		rateLimitService: NewRateLimitService(repo, nil, &config.Config{}, nil, nil),
	}
...
func TestOpenAIStreamingResponseFailedBeforeOutputOverloadedMessageReturnsFailover(t *testing.T) {
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-overloaded-message-failed"}},
	}
...
func TestOpenAIStreamingTooLongBeforeSemanticOutputReturnsResponseTooLarge(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
