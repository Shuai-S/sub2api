...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			...
			if s.service.isOpenAIAccountRuntimeBlocked(selection.Account) || !groupCompatible ||
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account == nil || s.service.isOpenAIAccountRuntimeBlocked(account) || !s.service.openAIAccountMatchesSchedulingGroup(account, req.GroupID) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport) {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if !s.service.accountSupportsOpenAIRequestCapabilities(account, req.RequiredCapability, req.RequiredImageCapability, req.RequireImageStream) {
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, "", "", false, requireCompact, PlatformOpenAI, false, true)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, "", false, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	clientStream := len(imageStream) > 0 && imageStream[0]
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, clientStream, false, PlatformOpenAI, false, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, clientStream, false, PlatformOpenAI, false, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, imageStream, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, imageStream, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				...
				if s.accountSupportsOpenAIRequestCapabilities(selection.Account, requiredCapability, requiredImageCapability, imageStream) {
					return selection, decision, nil
				}
				if selection.ReleaseFunc != nil {
					selection.ReleaseFunc()
		...
		for {
			...
			if s.isOpenAIAccountTransportCompatible(selection.Account, requiredTransport) &&
				s.accountSupportsOpenAIRequestCapabilities(selection.Account, requiredCapability, requiredImageCapability, imageStream) {
				return selection, decision, nil
			}
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		RequiredCapability:      requiredCapability,
		RequiredImageCapability: requiredImageCapability,
		RequireImageStream:      imageStream,
		RequireCompact:          requireCompact,
		ExcludedIDs:             excludedIDs,
	})
}
