...
// TestPassthroughIngressCallsBeforeTurn 钉死 ws_v2 passthrough 与 handler 的
// turn 生命周期契约：每轮请求在提交用量前先触发 BeforeTurn，让 handler 能按
// 当前时刻复核利润门并冻结该 turn 的 pricingAt。
func TestPassthroughIngressCallsBeforeTurn(t *testing.T) {
	...
	require.Equal(t, 1, gotBefore, "首轮 passthrough 请求应且只应触发一次 BeforeTurn")
	require.Positive(t, gotAfter, "passthrough terminal event 应触发 AfterTurn 提交用量")
}
