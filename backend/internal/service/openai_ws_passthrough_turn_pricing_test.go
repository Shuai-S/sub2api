...
// TestPassthroughIngressCallsBeforeTurnForInitialTurn pins the initial turn
// lifecycle contract: BeforeTurn runs before usage is finalized and the
// terminal event invokes AfterTurn.
func TestPassthroughIngressCallsBeforeTurnForInitialTurn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	controlCtx, cancelControl := context.WithCancelCause(context.Background())
	defer cancelControl(context.Canceled)

	upstream := newStagedPassthroughConn()
	upstream.Send(`{"type":"response.completed","response":{"id":"resp_initial_pricing","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`)
	...
	hooksMu.Unlock()
	require.Equal(t, 1, gotBefore, "initial passthrough request should trigger exactly one BeforeTurn")
	require.Positive(t, gotAfter, "passthrough terminal event should trigger AfterTurn")
}
