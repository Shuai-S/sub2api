...
// callers while the local scheduler uses the more precise semantic wording.
...
func openAIStreamAddedEventStartsClientOutput(payload []byte, eventType string) bool {
	...
	case "response.output_item.added":
		...
		case "function_call", "custom_tool_call":
			// The item identity itself commits this attempt even before arguments
			// or input deltas arrive. It is structural output, but not visible TTFT.
			return true
		case "compaction":
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithModel(
	...
	failoverErr.UpstreamErrorCode = upstreamErrorCode
	failoverErr.UpstreamErrorType = upstreamErrorType
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
				...
				if !outputStarted {
					...
					if shouldFailover {
						return resultWithUsage(),
							openAIStreamPreSemanticFailover(s.newOpenAIStreamFailoverErrorWithModel(c, account, true, upstreamRequestID, dataBytes, failedMessage, mappedModel, resp.Header))
					}
			...
			if (eventType == "response.completed" || eventType == "response.done") &&
				!sawFailedEvent && !semanticOutputSeen && !openAIStreamSemanticOutputStarted(c, semanticOutputStarted) &&
