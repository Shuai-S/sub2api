...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	for {
		...
		var completedTurnErr error
		var responseFailedErr *openAIUpstreamResponseFailedError
		if result != nil && errors.As(relayErr, &responseFailedErr) &&
			strings.EqualFold(responseFailedErr.Code, openAIWSFallbackReasonInvalidEncryptedContent) {
			// The failed turn was delivered. Keep the connection for the next
			// turn's lineage cleanup, while reporting this turn as a failure.
			completedTurnErr = responseFailedErr
			relayErr = nil
		}
		if relayErr != nil {
			lastTurnClean = false
		...
		lastTurnClean = true
		if hooks != nil && hooks.AfterTurn != nil {
			hooks.AfterTurn(turn, result, completedTurnErr)
		}
		if result == nil {
