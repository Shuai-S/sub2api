...
func TestDefaultOpenAIAdaptiveSchedulerSettingsMatchSharedCore(t *testing.T) {
	...
	require.Equal(t, 30, cfg.OpenAIAdaptiveSchedulerLearningMinHealthSamples)
	require.Equal(t, 0.05, cfg.OpenAIAdaptiveSchedulerSuccessEMAAlpha)
	require.Equal(t, 0.05, cfg.OpenAIAdaptiveSchedulerTTFTEMAAlpha)
	require.Equal(t, 3, cfg.OpenAIAdaptiveSchedulerHealthFailureThreshold)
	...
	require.Equal(t, 0.50, cfg.OpenAIAdaptiveSchedulerWeightSuccess)
