...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if err == nil {
			h.gatewayService.ReportOpenAIAccountScheduleSuccessWithContext(c.Request.Context(), account.ID, result, account.GetMappedModel(requestedModel))
			if result != nil {
		...
		if !errors.As(err, &failoverErr) {
			h.gatewayService.ReportOpenAIAccountAdaptiveFailureTerminalWithContext(c.Request.Context(), account.ID, err, nil, time.Since(forwardStart).Milliseconds(), false)
		...
		h.gatewayService.ReportOpenAIAccountAdaptiveFailureTerminalWithContext(c.Request.Context(), account.ID, failoverErr, nil, time.Since(forwardStart).Milliseconds(), false)
		if c.Writer.Size() != writerSizeBeforeForward {
