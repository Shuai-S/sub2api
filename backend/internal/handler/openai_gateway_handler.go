...
//
// 零值表示当前 turn 尚未进入 BeforeTurn。正常 ingress 会逐 turn 触发
// BeforeTurn 并完成利润复核；透传若未冻结，则用 TurnStarted 的所属 turn 时刻
// 回退计价，绝不能使用建连时刻，以免长连接跨峰谷时错误沿用旧价格。
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	for {
		...
		// AfterTurn 读取所属 turn 的时刻；未冻结时回退到 TurnStarted 的时间。
