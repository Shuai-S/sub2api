...
func TestOpenAIResponses_400ModelNotFoundSwitchesAccountWithoutAdaptiveFailureSample(t *testing.T) {
	...
	// The fallback account returns a deterministic upstream 400. Native
	// Responses now preserves that client error instead of normalizing it to 502.
	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Equal(t, "invalid_request_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
	require.Equal(t, "terminal test error", gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
}
